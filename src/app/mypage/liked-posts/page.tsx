  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase
        .from('post_likes')
        .select('post:posts(id, title, type, created_at, club_id, club:clubs(id, name), comments(count), post_likes(count))')
        .eq('user_id', user.id)
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 pb-24">
        <div className="sticky top-0 bg-gray-50 z-10 flex items-center gap-3 py-4">
          ...
          <h1 className="font-bold text-lg text-gray-900">내가 좋아요한 글</h1>
        </div>

        {loading ? (
          <p className="mt-16 text-center text-sm text-gray-400">불러오는 중...</p>
        ) : posts.length === 0 ? (
          <p className="mt-16 text-center text-sm text-gray-400">좋아요한 글이 없습니다.</p>
        ) : (
          <div className="flex flex-col gap-2">
            {posts.map(post => (
              <Link
                key={post.id}
                href={`/clubs/${post.club_id}/board/${post.id}?returnTo=/mypage/liked-posts`}
