    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 pb-24">
        <div className="sticky top-0 bg-gray-50 z-10 flex items-center justify-between py-4">
          ...
          {isStaff && <Button size="sm" onClick={() => setShowForm(v => !v)}><Plus size={15} />내역 추가</Button>}
        </div>
        ...
        {showForm && (
          <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-4 flex flex-col gap-3">
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1" loading={loading} onClick={handleAdd}>저장</Button>
            </div>
          </div>
        )}
          ...
          {finances.length > 0 ? finances.map((f: Finance) => (
            <div key={f.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0
                ...
                {f.type === 'income' ? <TrendingUp size={18} className="text-green-600" /> : <TrendingDown size={18} className="text-red-500" />}
              </div>
